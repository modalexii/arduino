...
const int receive_pin = 11; // PCB uses 11
...
bool try_rx() {
  ...
  delay(60); // 52 seems to be the minimum here, 60 gives some tolerance
  bool rx_successful = false;
  uint8_t buf[VW_MAX_MESSAGE_LEN];
  for (uint8_t i = 0; i < 2; i++) { // try twice
    if (vw_get_message(buf, VW_MAX_MESSAGE_LEN)) {
