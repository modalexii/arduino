...
byte serverIP[]= { 10,0,0,57 };        // the ip to send data, 255,255,255,255 is broadcast sending
// art net parameters
unsigned int artNetPort = 6454;      // artnet UDP port is by default 6454
const int remoteUniverse=0;//universe is from 0 to 15, subnet is not used
const int numChannels=512; //512 for 512 channels, MAX=512

//HARDWARE
byte mac[] = {  144, 162, 218, 00, 16, 96  };//the mac adress of ethernet shield or uno shield board
byte ip[] = { 10,0,0,11 };// the IP adress of your device, that should be in same universe of the network you are using, here: 192.168.1.x
...
void initilizeANHeader()
{
    ...
    //Operator code low byte first  
     ...
     ArtDmxBuffer[13] = 0;
     // universe 
     ArtDmxBuffer[14]= remoteUniverse;//or 0
     ArtDmxBuffer[15]= remoteUniverse>> 8;
     //data length
     ArtDmxBuffer[16] = numChannels>> 8;
